...
public interface JobRepository extends CrudRepository<Job,Long> {
    ...
    @Modifying
    @Query(value = "UPDATE schedules SET status = :status,start_time = :start_time,next_time = :next_time,end_time = :end_time,cron_expression = :cron WHERE id = :id",nativeQuery = true)
    ...
    @Transactional(rollbackOn = Exception.class)
    @Modifying
    @Query(value = "UPDATE schedules SET next_time = :next_time WHERE id = :id AND next_time < :next_time",nativeQuery = true)
